...
def softmax_weights(dist, mask):
    max_v = paddle.max(dist * mask, dim=1, keepdim=True)[0]
    diff = dist - max_v
    Z = paddle.sum(paddle.exp(diff) * mask, dim=1, keepdim=True) + 1e-6  # avoid division by zero
...
def weighted_example_mining(dist_mat, is_pos, is_neg):
    ...
    dist_ap = paddle.sum(dist_ap * weights_ap, dim=1)
    dist_an = paddle.sum(dist_an * weights_an, dim=1)
